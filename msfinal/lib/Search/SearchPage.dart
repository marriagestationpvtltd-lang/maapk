...
class _SearchPageState extends State<SearchPage>
  ...
  Future<void> _fetchBlockedUsers() async {
    try {
      ...
      final response = await http.post(
        Uri.parse('http://10.0.2.2/Api2/get_blocked_users.php'),
        headers: {'Content-Type': 'application/json'},
  ...
  Future<void> _fetchInitialTotalCount() async {
    ...
    try {
      final url = Uri.parse(
          'http://10.0.2.2/Api2/search_opposite_gender.php?user_id=$_currentUserId');
      final response = await http.get(url);
      if (response.statusCode == 200) {
        final result = jsonDecode(response.body);
        if (result['success'] == true) {
  ...
  void _fetchMatchesCount() {
    ...
    _debounceTimer = Timer(const Duration(milliseconds: 500), () async {
      ...
      try {
        ...
        final url = Uri.parse(
            'http://10.0.2.2/Api2/search_opposite_gender.php?$queryString');
        final response = await http.get(url);
        if (response.statusCode == 200) {
          final result = jsonDecode(response.body);
          if (result['success'] == true) {
